...
Groups can be nested, so you can build expressions like "environment is production AND (region is sg OR region is us-east-1)". Between each condition in a group, a badge shows the logic (**AND** or **OR**) so the evaluation order is always visible.
